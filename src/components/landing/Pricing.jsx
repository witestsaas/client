  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle at top right, rgba(242, 183, 5, 0.08) 0%, transparent 60%), radial-gradient(circle at bottom left, rgba(222, 222, 222, 0.06) 0%, transparent 60%)',
        }}
      />
        ...
        >
          {plans.map((plan) => (
            <motion.div
              key={plan.name}
              variants={fadeUp}
              whileHover={{ y: -12, boxShadow: `0 30px 60px rgba(255,183,51,0.25)` }}
              whileTap={{ scale: 0.98 }}
              className="relative rounded-2xl p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 transition-all duration-300 cursor-pointer"
              style={
                plan.highlight
                  ? {
                      background: 'linear-gradient(135deg, #F29F05 0%, #ff8c00 100%)',
            ...
            >
              {plan.highlight && (
               <div
                  className="absolute -top-3 left-1/2 -translate-x-1/2 text-[#F29F05] text-xs font-bold px-3 py-1 rounded-full"
                  style={{
              ...
              <Link to="/signup">
                <button
                  className={`w-full py-3 rounded-xl font-semibold text-sm transition-colors duration-200 ${
                    plan.highlight ? 'bg-black text-[#F29F05] hover:bg-[#111]' : ''
                  }`}
                  style={
                    !plan.highlight
                      ? {
